...
//& COMPLETE PROFILE
const completeProfile = async (payload, user) => {
    ...
    if (skills !== undefined) {
        ...
        if (existingSkills.length !== skills.length) {
            ...
            throw new AppError(httpStatus.NOT_FOUND, `Skill not found: ${missingIds.join(", ")}`);
        }
    }
    if (region !== undefined) {
        ...
        if (existingRegions.length !== region.length) {
            ...
            throw new AppError(httpStatus.NOT_FOUND, `Region not found or inactive: ${missingIds.join(", ")}`);
        }
    }
    const transactionResult = await prisma.$transaction(async (tx) => {
        ...
        const update = await tx.technicianProfile.update({
            ...
            data: {
                ...
                ...(region !== undefined && { regions: { set: region.map((id) => ({ id })) } }),
            },
        });
        ...
        if (availability !== undefined && availability.length > 0) {
            ...
            const oneOffOrBlocked = availability.filter(s => s.type !== "RECURRING");
            ...
            for (const slot of recurring) {
                await tx.availability.upsert({
                    ...
                    update: {
                        startTime: slot.startTime ? new Date(`1970-01-01T${slot.startTime}:00Z`) : null,
                        endTime: slot.endTime ? new Date(`1970-01-01T${slot.endTime}:00Z`) : null,
                        isActive: true
                    },
                    create: {
                        technicianId: isTechnician.id,
                        type: "RECURRING",
                        dayOfWeek: slot.dayOfWeek,
                        startTime: slot.startTime ? new Date(`1970-01-01T${slot.startTime}:00`) : null,
                        endTime: slot.endTime ? new Date(`1970-01-01T${slot.endTime}:00`) : null,
                    }
                });
            }
    ...
    const isNowComplete = !!fullProfile.phone &&
    ...
    const formattedAvailability = fullProfile?.availability.map(slot => ({
        ...slot,
        startTime: slot.startTime ? new Date(slot.startTime).toISOString().substring(11, 16) : null,
        endTime: slot.endTime ? new Date(slot.endTime).toISOString().substring(11, 16) : null,
    }));
