...
const verifyEmail = async (payload) => {
    ...
    const html = await ejs.renderFile(path.join(process.cwd(), "src/app/template/welcome.ejs"), templateData);
    await transporter.sendMail({
        from: config.smtp_sender,
...
const forgotPassword = async (payload) => {
    ...
    const templatePath = path.join(process.cwd(), "src/app/template/forgot-password-otp.ejs");
    const templateData = {
...
const resetPassword = async (payload) => {
    ...
    const hashPass = await bcrypt.hash(newPassword, Number(config.bcrypt_salt_rounds));
    ...
    const html = await ejs.renderFile(path.join(process.cwd(), "src/app/template/reset-password.ejs"), templateData);
    await transporter.sendMail({
        from: config.smtp_sender,
        to: isExistUser.email,
