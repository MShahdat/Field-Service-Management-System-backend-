...
//& APPLY MANAGER
const applyManager = async (payload) => {
    ...
    if (regionIds.length > 0) {
        ...
        if (invalidIds.length > 0) {
            throw new AppError(httpStatus.BAD_REQUEST, `Invalid region ID(s): ${invalidIds.join(", ")}. These regions do not exist.`);
        }
    }
    ...
    const templatePath = path.join(process.cwd(), "src/app/template/verification-otp.ejs");
    const templateData = {
...
const emailVerify = async (payload) => {
    ...
    if (!redisOTP) {
        throw new AppError(httpStatus.BAD_REQUEST, "otp expired. please apply again");
    }
    if (redisOTP !== otp) {
        throw new AppError(httpStatus.BAD_REQUEST, "OTP mismatch. please give valid OTP");
    }
...
const approveManager = async (payload, reviewer) => {
    ...
    if (!isManager.emailVerified) {
        throw new AppError(httpStatus.BAD_REQUEST, "Manager email not verified yet");
    }
    ...
    if (isManager.manager?.verificationStatus !== "PENDING") {
        throw new AppError(httpStatus.CONFLICT, `you can't update varification status from  '${isManager.manager?.verificationStatus.toLowerCase()}'.`);
    }
    if (verificationStatus === "REJECTED" && !rejectionReason) {
        throw new AppError(httpStatus.BAD_REQUEST, "for rejection must need to rejection reason");
    }
    ...
    const templatePath = path.join(process.cwd(), `src/app/template/${templateName}`);
    const templateData = {
        name: isManager.name,
